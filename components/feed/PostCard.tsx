...
export default function PostCard({ post, authorNick, authorAvatar }: Props) {
  ...
  /**
   * Usuwa dokument posta z Firestore.
   ...
   *
   * Zdjęcia na Cloudinary NIE są usuwane (patrz komentarz w ciele funkcji).
   */
  async function handleDelete() {
    if (!window.confirm(t.post.confirmDelete)) return;
    setDeleted(true); // natychmiast ukryj kartę — nie czekaj na Firestore
    try {
      // Zdjęcia (Cloudinary) NIE są usuwane — brak bezpiecznego unsigned-delete API
      // bez serwera. Osierocone pliki zostają na koncie (mieści się w darmowym 10GB).
